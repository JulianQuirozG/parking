...
@Injectable()
export class findOneDataParking implements CanActivate {
  constructor(
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (user.role === ROL.SOCIO) {
      const record = await this.currentParkingsService.findOne(+id);
      const parking = await this.parkingsService.findOne(record.parking.id);
